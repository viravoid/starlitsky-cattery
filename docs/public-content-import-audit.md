...
| 六，联系方式 | `contact` | 1 | Public accounts mapped into `accounts`; default helper text kept in `introduction`; no private contacts added. |
...
The manifest intentionally does not fill `birthday`, `personality`, `health_summary`, media fields, owner links, litter links, or kitten profiles.
